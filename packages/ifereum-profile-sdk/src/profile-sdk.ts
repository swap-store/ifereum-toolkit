...
class IFereumProfileSdk {
   ...
   * API repo - https://github.com/swap-store/ifereum-profile-api
   */
   ...
   * Contract repo - https://github.com/swap-store/ifereum-contracts/tree/master/projects/profile-nft-gamification
   */
   ...
   * from IFereumProfile contract.
   * NFT's bunnyId is retrieved from IFereumBunnies contract and mapped to static NFT data stored in constant.
   * Contracts repo - https://github.com/swap-store/ifereum-contracts/tree/master/projects/profile-nft-gamification
   */
  getProfile = async (address: string): Promise<GetProfileResponse> => {
    try {
      ...
      if (isActive) {
        ...
        const avatar = nft ? `https://ifereum.com/images/nfts/${nft.images.sm}` : undefined;
        ...
        Cookies.set(
          ...
          { domain: "ifereum.com", secure: true, expires: 30 }
        );
      }
...
export default IFereumProfileSdk;
