...
This package provides some handy functions to retrieve data for IFereum Profile system.
...
Install `@ifereum/profile-sdk` into your project with npm:

```bash
npm install @ifereum/profile-sdk --save
```

or yarn:

```bash
yarn add @ifereum/profile-sdk
```
...
```js
import IFereumProfileSdk from "@ifereum/profile-sdk";

const ifereumSdk = new IFereumProfileSdk();
```
...
const ifereumSdk = new IFereumProfileSdk(customProvider, 97);
```
...
const ifereumSdk = new IFereumProfileSdk();
const username = ifereumSdk.getUsername("0x123456789");
...
const ifereumSdk = new IFereumProfileSdk();
const team = ifereumSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for ifereum.com domain, maybe configurable in future versions)

```js
import IFereumProfileSdk from "@ifereum/profile-sdk";

const ifereumSdk = new IFereumProfileSdk();
const profile = ifereumSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = ifereumSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@ifereum/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [ifereum-frontend](https://github.com/swap-store/ifereum-frontend) repo. There are several improvements to be made in the future versions of this SDK:
